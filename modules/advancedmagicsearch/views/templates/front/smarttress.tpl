        ...
        <div id="input-container">
            {foreach from=$questions item=question key=q}
                ...
                {foreach from=$answersutility[$q] key=ai item=answeritem}
                    {if $last_selected == 0}
                        <div>
                            <input type=radio name=group_{$q} class="checkclass" value={$ai} checked data_label_id="risposta_{$q}_{$ai}" data_label_question_id="domanda_{$q}">
                    ...
                    {else}
                        <div>
                            <input type=radio name=group_{$q} class="checkclass" value={$ai} data_label_id="risposta_{$q}_{$ai}" data_label_question_id="domanda_{$q}"> 
                            <label id="risposta_{$q}_{$ai}">
                                {$answers[$answeritem]}
                            </label>
                        </div>
